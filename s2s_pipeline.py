...
from arguments_classes.style_tts_arguments import StyleTTSHandlerArguments
import torch
import nltk
...
def parse_arguments():
    parser = HfArgumentParser(
        (
            ...
            StyleTTSHandlerArguments,  # Added
...
def prepare_all_args(
    ...
    style_tts_handler_kwargs,  # Added
):
    prepare_module_args(
        module_kwargs,
        whisper_stt_handler_kwargs,
    ...
    rename_args(style_tts_handler_kwargs, "style_tts")  # Added
...
def build_pipeline(
    ...
    tts = get_tts_handler(module_kwargs, stop_event, lm_response_queue, send_audio_chunks_queue, should_listen, parler_tts_handler_kwargs, melo_tts_handler_kwargs, chat_tts_handler_kwargs , style_tts_handler_kwargs)  # Added
...
def get_tts_handler(module_kwargs, stop_event, lm_response_queue, send_audio_chunks_queue, should_listen, parler_tts_handler_kwargs, melo_tts_handler_kwargs, chat_tts_handler_kwargs , style_tts_handler_kwargs):
    ...
    elif module_kwargs.tts == "style":
        try:
            from TTS.styleTTS_handler import StyleTTSHandler
        except RuntimeError as e:
            logger.error("Error importing StyleTTSHandler")
            raise e
        return StyleTTSHandler(
            stop_event,
            queue_in=lm_response_queue,
            queue_out=send_audio_chunks_queue,
            setup_args=(should_listen,),
            setup_kwargs=vars(style_tts_handler_kwargs),
        )
    else:
        raise ValueError("The TTS should be either parler, style, melo or chatTTS")
...
def main():
    ...
    prepare_all_args(
        ...
        melo_tts_handler_kwargs,
        chat_tts_handler_kwargs,
        style_tts_handler_kwargs,  # Add this line
